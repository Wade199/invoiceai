...
|---|-------|----------|-------------|------------|
| 15 | P4 — UI Streamlit (4 écrans) — **socle et écran Upload faits le 2026-09-21** (config, client API, `safe()`, navigation, page Upload, lanceur `python scripts/run.py`, revue `docs/security/P4_UI_REVIEW.md`) ; reste : pages Résultat, Historique, Export ; ; `escape_markdown()` sur chaque champ venant du LLM ; barre de progression (7-33 s / facture) | High | #13b | 2-3j |
